...
La mayoría de los trabajos realizados sobre Estimación de Pose usan \textit{Imágenes RGB} debido
a su alta popularidad y fácil captura y acceso, en estos últimos años y debido a la masificación
de dispositivos inteligentes el acceso a una cámara digital no resulta un problema para la mayoria.
Sin embargo, esto solo es cierto para la Estimación de Pose 2D puesto que el etiquetado de datos
usando las coordenadas de la imagen como referencia no representa demasiado problema. En el caso
de la Estimación de Pose 3D aún se necesita usar equipo costoso de captura de movimiento en
acompañamiento de las cámaras de video. Existen dos tipos principales los ópticos y no ópticos.
Los primeros en general el sujeto de prueba no necesita de aditamentos complejos como el uso de trajes
(exoesqueleto) y solo un software y cámaras con sensores (infrarojos) se
encarga de registrar e interpretar el movimiento a un modelo digital. Si bien
aventajan en una reducción de coste su precisión es menor. El segundo tipo se
puede dividir en dos clases; los mecánicos que usan giroscopios y acelerómetros para registrar el
movimiento y electromagnéticos los cuales generan un campo electromagnético y capturan las alteraciones
en este que realiza el sujeto al moverse.
